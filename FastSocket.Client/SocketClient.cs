...
namespace FastSocket.Client
{
        ...
        where TMessage : class, Messaging.IMessage
    {
        ...
        public SocketClient(Protocol.IProtocol<TMessage> protocol,
            ...
            int millisecondsReceiveTimeout) : base(socketBufferSize, messageBufferSize)
        {
        ...
        /// <returns></returns>
        public bool TryRegisterEndPoint(string name, EndPoint[] arrRemoteEP,
            Func<SocketBase.IConnection, Task> initFunc = null)
        {
        ...
        /// <returns></returns>
        public Request<TMessage> NewRequest(string name, byte[] payload, int millisecondsReceiveTimeout,
        ...
        protected virtual void OnReceived(SocketBase.IConnection connection, Request<TMessage> request,
            TMessage message)
        {
            ThreadPool.QueueUserWorkItem(_ =>
            {
        ...
        protected override void OnMessageReceived(IConnection connection, MessageReceivedEventArgs e)
        {
            ...
            try
            {
                message = this._protocol.Parse(connection, e.Buffer, out readlength);
            }
            catch (Exception ex)
            {
        ...
        private class PendingSendQueue
        {
            ...
            public PendingSendQueue(SocketClient<TMessage> client)
            {
                ...
                this._timer = new Timer(state =>
                {
                    ...
                    var timeOut = this._client._millisecondsSendTimeout;
                    while (count-- > 0)
                    {
            ...
            public ReceivingQueue(SocketClient<TMessage> client)
            {
                ...
                this._timer = new Timer(_ =>
                {
                    ...
                    var arr = this._dic.ToArray().Where(c =>
                            dtNow.Subtract(c.Value.SentTime).TotalMilliseconds > c.Value.MillisecondsReceiveTimeout)
                        .ToArray();
        ...
        private class NodeInfo
        {
            ...
            public NodeInfo(string name, EndPoint[] arrRemoteEP, Func<SocketBase.IConnection, Task> initFunc)
            {
        ...
        private class Node : IDisposable
        {
            ...
            {
                SocketConnector
                    .Connect(this.Info.ArrRemoteEP.Length == 1
                        ? this.Info.ArrRemoteEP[0]
                        : this.Info.ArrRemoteEP[
                            (Guid.NewGuid().GetHashCode() & int.MaxValue) % this.Info.ArrRemoteEP.Length])
